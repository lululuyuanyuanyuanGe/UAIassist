...
【数据源说明】
1. 核心数据源：标记为"=== 核心数据源：xxx ==="的数据，这是你进行数据合成的主要依据
2. 参考数据源：标记为"=== 参考数据源：xxx ==="的数据，用于补充和验证核心数据源中的信息
3. 补充信息和上下文：标记为"=== 补充信息和上下文 ==="的内容，用于理解业务背景和填充规则

【核心工作原则】
⭐ 优先使用核心数据源：所有数据合成和填充都应以核心数据源为基础
⭐ 参考数据源作为补充：当核心数据源缺失信息时，可从参考数据源中查找补充
⭐ 保持数据一致性：确保生成的数据与核心数据源的逻辑和格式保持一致
...
第二层推理：逐列字段推理
对于每个目标字段，进行以下链式推理：
1. 字段定义理解：这个字段要求什么类型的数据？
2. 数据源定位：在核心数据源中如何找到对应信息？
3. 数据匹配验证：找到的数据是否符合字段要求？
4. 补充数据查找：如果核心数据源不足，参考数据源中有什么？
5. 数据转换处理：需要进行什么格式转换或计算？
6. 合理性验证：最终确定的数据是否逻辑合理？

【推理步骤】
请严格按照以下步骤进行推理，并展示每一步的思考过程：

步骤1：数据源全面分析
- 识别核心数据源、参考数据源和补充信息的内容
- 理解各数据源的作用和优先级
- 预估数据处理的复杂度和潜在问题
...
【数据行 X 的处理】
→ 行数据识别：当前处理的是第X行，包含哪些原始信息？
→ 逐列字段推理：
  ├── 字段1：[字段名称]
  │   ├── 定义理解：该字段要求什么？
  │   ├── 数据定位：在核心数据源中找到 [具体数据]
  │   ├── 数据验证：该数据是否符合要求？
  │   ├── 补充查找：（如需要）参考数据源中有 [补充信息]
  │   ├── 转换处理：需要进行 [具体转换]
  │   └── 最终确定：该字段的值为 [最终值]
  ├── 字段2：[字段名称]
  │   ├── 定义理解：该字段要求什么？
