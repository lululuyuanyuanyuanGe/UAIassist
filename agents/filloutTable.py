...
【CSV列生成规则】
🎯 **重要**：所有在JSON结构中出现的字段名（无论是父字段还是子字段）都必须作为CSV的列输出：
1. **父字段**：如果有"分解"，父字段本身也要作为一列（如"保障人数"）
...
2. 列生成判断：该字段是否需要作为CSV列？是否有子字段也需要作为列？
...
[仅输出纯净的 CSV 数据行，使用英文逗号分隔]
  ...
  ✅ 正确处理父字段和子字段的关系，所有字段都要作为CSV列
  ✅ 严格执行"规则"中定义的计算关系
  ✅ 最终答案仅包含CSV数据，不含任何其他内容
  ✅ 字段顺序必须按照结构化映射的自然顺序（父字段，再其子字段）
  ✅ 严禁遗漏字段、重复字段或输出空值
