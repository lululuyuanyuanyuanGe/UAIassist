...
with open(r"D:\asianInfo\ExcelAssist\agents\images\Screenshot 2025-07-14 172825.png", "rb") as image_file:
...
1. **提取完整的表头层级结构**
   ...
   - 使用嵌套的 key-value 结构，表达表头的层级关系；
   - 每一级表头应清晰反映其子级字段或子分类，避免遗漏或误分类。

2. **识别分类汇总关系**
   - 如果某个上级字段是其下级子字段的**数值汇总**，请在结构中明确体现这种逻辑；
   - 汇总关系格式：
     "汇总字段": "子字段1 + 子字段2 + ..."
   - 子字段继续列出，内容为 []（表示用于填数据）。

3. **特别注意：父级字段同时具有数据和分类子字段**
   - 如果某个父级字段（如：保障人数、领取金额）**自身单元格有数据，同时又包含多个子字段**（如重点保障人数、残疾人数），必须将父级字段的数据**单独保留为一个字段**，与子字段并列输出。
   - 此时结构应为：
     "父级字段": {
       "自身数据": [],
       "子字段1": [],
...
4. **辅助判断字段含义**
   - 根据数据内容辅助理解字段用途，避免仅依赖表头文字表面含义；
   - 识别重复字段、并列字段、合并单元格带来的结构层级；
   - 发现存在数据结构对齐但表头描述模糊的情况，尽量根据数据判断其真实分类。

【输出格式】

- 严格输出标准 JSON，不允许有 markdown、代码块标记或多余的文字；
- 输出仅包含表头结构，不输出任何总结、描述或用途说明；
- 输出 JSON 的根键名必须为 {file_name}（严格保留，不能更改）；
- 每一层级都以对象形式描述，结构示例如下：
...
- **父级字段如果自身有数据，必须单独保留其数据字段。**
- 输出必须为严格的 JSON 格式，且仅包含表头结构。
- 不允许输出表格总结、描述、用途等其他信息。
- 不允许有 markdown 包裹、代码块标记或多余的文字。
- 保持层级结构清晰，完整保留所有分类表头、子表头和字段表头。
- **结合数据辅助判断表头结构，确保分类、层级、汇总关系准确。**
""")
