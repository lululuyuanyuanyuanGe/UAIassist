...
3. **处理父字段的"值"字段**：
   对于具有嵌套结构的复杂字段（包含"值"、"分解"、"规则"的字段），请注意：
   - 如果模板中父字段的"值"为空数组 [] ，说明该父字段本身不需要填入数据值
   - 这种情况下，应该保持"值"为空，不要为其分配数据源映射
   - 只需要为"分解"中的子字段建立数据源映射关系
   - 当"值"字段包含规则，或者数据来源时，需要把规则或者数据来源写出来，此时我们需要映射关系

4. **输出格式要求**：  
   返回结果应保持与原模板表格结构一致，但每个表头需扩展为以下形式之一：
   - `来源文件名: 数据字段名`（表示该字段来自数据文件）
   - 如果一个表头的来源可能来自多个文件，请把所有可能的来源文件名和数据字段名都写在同一行，用/分割
   - `推理规则: ...`告诉模型该字段需要通过什么逻辑推导出来，必须把详细的规则，或者计算公式写出来，不要遗漏
   - **重要**: 对于复杂字段的"值"部分，如果原模板中为空数组[]，必须保持为空数组[]，不要添加任何映射
